        ...
        <div class="body-wrapper-inner">
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h4 class="m-0">View Purchase Order: {{$order->chalan_reg}}</h4>
                </div>
                            ...
                            <table class="table table-bordered table-striped align-middle">
                                ...
                                <tbody>
                                    @foreach($cart as $key => $val)
                                        <tr>
                                            <td class="text-center"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">{{ $key + 1 }}</td>
                                            <td  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">{{ $val->medicine->name }}</td>
                                            <td class="text-center"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">৳{{ number_format($val->purchase_price, 2) }}</td>
                                            <td class="text-center"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">{{ $val->order_qty }}</td>
                                            <td class="text-center"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">{{ $val->delivery_qty }}</td>
                                            <td class="text-end text-success"  data-bs-toggle="modal" data-bs-target="#staticBackdrop{{$val->id}}">
...
<div class="modal fade" id="staticBackdrop{{$val->id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="{{url('/confirm-purchase-order-qty')}}" method="POST">
                @csrf
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">{{ $val->medicine->name }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">                    
                    <label for="">Order Qty: {{ $val->order_qty }}</label>
                    <input type="number" name="txtDeliveryQty" class="form-control" placeholder="Enter delivery qty." min="0">
                    <input type="hidden" value="{{$val->chalan_reg}}" name="txtReg">
                    <input type="hidden" value="{{$val->medicine_id}}" name="txtMedicine">
                </div>
